 ...
 def _wait_for_connections(self):
     ...
     while True:
         ...
         if (request_method == 'GET') | (request_method == 'HEAD'):
             ...
             #This is the only part i added to the server. I check if the parsed filename is update
             #this means that the request is for updating the client's page. So we take the input in terminal
             #from the user and create a server_response containing the string that was entered in the terminal
             if file_requested.split('/')[1] == 'update':
                response_headers = self._gen_headers( 200)
                server_response = response_headers.encode()
                inp=input("Enter update:")
                response_content = inp.encode()
